...
(function ($){
    ...
    TimelineMe.prototype = {
        ...
        init: function() {
            ...
            if(this.settings.scrollArrows == true) {
                ...
                if(this.settings.leftArrowElm == undefined) {
                    leftScroll = $('<i class="fa fa-chevron-circle-left fa-3x fa-timeline-me-leftarrow" aria-hidden="true"></i>');
                } else {
                    leftScroll = $('<i class="fa fa-chevron-circle-left fa-3x fa-timeline-me-leftarrow" aria-hidden="true"></i>');
                    leftScroll.html(this.settings.leftArrowElm);
                }
                if(this.settings.rightArrowElm == undefined) {
                    rightScroll = $('<i class="fa fa-chevron-circle-right fa-3x fa-timeline-me-rightarrow" aria-hidden="true"></i>');
                } else {
                            rightScroll = $('<i class="fa fa-chevron-circle-right fa-3x fa-timeline-me-rightarrow" aria-hidden="true"></i>');
            ...
            if(this.settings.items && this.settings.items.length > 0) {
                ...
                for(var i = 0; i < this.content.length; i++) {
                    ...
                    track.append(this._createItemElement(this.content[i], color));
                    if(color==3){
                      color=0;
                    }else{
                      color+=1;
                ...
                if(hasNumberProperty(this.content, 'relativePosition')) {
                    if(this.settings.orientation == 'horizontal') {
                      this._calcRelativePosRatioWidth(this.content);
                      this.maxRelativeRatio = getMaxPropertyValue(this.content, 'relativePosRatio');
                      this._calcRelativeWidth(this.content, this.maxRelativeRatio);
                      this._addMissingRelativeWidth(this.content);
                      resolveContainerWidth(track);
                    }else{
        ...
        // Method that calculates ratio: item's height / relative pos diff with next item
        _calcRelativePosRatioWidth: function(items) {
            for(var i = 0; i < items.length; i++) {
                var width = items[i].element[0].getBoundingClientRect().width;
                var relPosDiff = items[i].diffWithNextRelativePos;
                var ratio = 0;
                if(!isNaN(width) && !isNaN(relPosDiff))
                    ratio = width / relPosDiff;
        ...
        _calcRelativeWidth: function(items, widthRatio) {
            if(!widthRatio) return;

            for(var i = 0; i < items.length; i++) {
                items[i].relativeWidth = widthRatio * (items[i].diffWithNextRelativePos ? items[i].diffWithNextRelativePos : 0);
                // console.log(items[i].relativeWidth);
            }
        },
        ...
        _addMissingRelativeWidth: function(items) {
            for(var i = 0; i < items.length; i++) {
                var widthToAdd = 0;
                if(!isNaN(items[i].relativeWidth)) {
                    var width = items[i].element[0].getBoundingClientRect().width;
                    widthToAdd = items[i].relativeWidth - width;
                    var marginBottom = parseInt(items[i].element.css('padding-right'))/$(window).width()*100 + ((widthToAdd-150) > 0 ? (widthToAdd-150)/$(window).width()*100 : 0);
                    if(marginBottom>100){
                      marginBottom = 45;
                    }
                    items[i].element.css('padding-right', marginBottom + '%');
                }
            }
        },
        ...
        _createItemElement: function(item, color) {
            var itemElm;
            switch(item.type) {
                case 'milestone':
            ...
            this._buildItemContent(item, color);
            this._fillItem(item);
        ...
        _buildItemContent: function(item, color) {
            if(!item || !item.element)
                return;

            var pixelsRegex = /[0-9]+px$/;
            var color_classes = ['blue', 'orange', 'pink', 'green'];
            // Following wrapper are only used in horizontal mode, in order to correctly display bigItems (with table display)
            var itemWrapper = $('<div class="timeline-me-item-wrapper '+color_classes[color]+'">');
            ...
            var labelText = $('<div class="label-text">');
            ...
            var pictoElm = $('<div class="timeline-me-picto"><div class="timeline-me-picto-outer2"><div class="timeline-me-picto-outer1"><div class="timeline-me-picto-inner">');
            item.pictoElement = pictoElm;

            labelWrapper.append(labelElm);
            var shortContentElm ='';
            if(item.shortContent){
              shortContentElm = $('<div class="timeline-me-shortcontent-milestone">');
              shortContentLine = $('<div class="timeline-me-shortcontent-milestone-line">');
              shortContentBall = $('<div class="timeline-me-shortcontent-milestone-ball">');

            }
              labelWrapper.append(labelElm);
            if(item.position == 'bottom'){
              itemWrapper.append(shortContentElm);
              itemWrapper.append(shortContentBall);

              itemWrapper.append(shortContentLine);
              itemWrapper.addClass('with-description-top');
            }else{
              labelWrapper.append(shortContentElm);
              labelWrapper.append(shortContentLine);
              labelWrapper.append(shortContentBall);
              labelWrapper.addClass('with-description-bottom');
        ...
        _fillItem: function(item) {
            if((item.label || item.labelTemplate) && item.labelElement) {
                item.label ? item.labelElement.html('<p>'+item.label+'</p>') : item.labelElement.html(loadHTMLFromFile(item.labelTemplate));
    ...
    var bindScrollTo = function(leftScrollElm, rightScrollElm, elmToScroll, scrollSpeed) {
        ...
        if( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
          leftScrollElm.on('touchstart', function() {
              var timer = setInterval(function() {
                  elmToScroll.scrollLeft(elmToScroll.scrollLeft() - scrollSpeed);
              }, 20);

              leftScrollElm.on('touchend', function() {
                  clearInterval(timer);
              });
          });
          rightScrollElm.on('touchstart', function() {
              var timer = setInterval(function() {
                  elmToScroll.scrollLeft(elmToScroll.scrollLeft() + scrollSpeed);
              }, 20);

              rightScrollElm.on('touchend', function() {
                  clearInterval(timer);
              });
          });
       }else{
